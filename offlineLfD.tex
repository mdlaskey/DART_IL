...
\adnote{We focus on Learning from Demonstration (LfD), in which a robot learns to perform a task via examples from a human supervisor \cite{abbeel2007application,abbeel2008apprenticeship,van2010superhuman,laskeyshiv,laskeyrobot}(need better cites for lfd, including argall survey). LfD can be tedious, requiring many demonstrations for each task \cite{}. Our ultimate goal is to improve the data efficiency in LfD, making it quick and easy to teach the robot new tasks. We take a step in this direction by analyzing two different approaches for acquiring supervisor data.}
...
In the supervised learning approach to LfD, \adnote{the robot learns a policy -- a mapping from the state space to controls -- } via regression or classification. The \adnote{standard} approach to LfD is to \adnote{let the supervisor provide multiple demonstrations of performing the task, which induces a set of state-control examples for} the states the supervisor is likely to visit~\cite{pomerleau1989alvinn}. We \adnote{refer to} this as \emph{Human-Centric} (HC) \adnote{sampling}, because the human controls what states the robot will observe. \adnote{Because of} error in learning, the robot's policy will not \adnote{perfectly }match the supervisor\adnote{, and will end up visiting new} states when performing the task. \adnote{An alternative approach to HC sampling} is to have the supervisor retroactively provide \adnote{corrections to the robot \cite{ross2010efficient,ross2010reduction,laskeyrobot,laskeyshiv,he2012imitation}, i.e. iteratively re-label the states that the robot visits with its current learned policy.}
...
These attempts to provide examples on states the robot is more likely to see.  We refer to this as Robot-Centric (RC) \adnote{sampling,} because the robot \adnote{dictates} the choice of \adnote{states that the supervisor labels}. 



\adnote{Our insight is that} while RC has great advantages, it comes at a cost \adnote{in practice}. One cost is the challenge posed to a \emph{human} supervisor.  RC methods have a human provide retro-active feedback to the robot without observing the effect of their suggested control. \adnote{Instead, the robot moves on to the next state where it asks for a label.}  
The human supervisor has to predict how the world will change given a control, instead of \adnote{operating the robot with a feedback loop like in HC. This makes labels much more noisy (Fig. \ref{fig:teaser}(b)).}
 \mlnote{motivate this with HRI related work} 

\adnote{A second cost is that labels on states that occur} due to previous errors \adnote{induce a more complex policy than standard demonstrations. These situations are common when the robot veers into a state where the recovery behavior adds complexity to the task: the supervisor has to show the robot complex recovery maneuvers that do not occur during normal task execution. Imagine, for instance, the task of singulating an object from a set of objects on a table (Fig. \ref{fig:teaser}. Standard demonstrations involve moving forward and from side to side to push obstacles out of the way and recover the desired object. But if the robot makes errors and ends up in a failure state (Fig. \ref{fig:teaser}(c,right)), then more complex maneuvers will be required. }
Adding these difficult examples to the dataset could reduce the robot's ability to match the supervisor. 

We explore the value of the extra overhead of RC \adnpte{in particular }for emerging classes of highly-expressive deep learning models such as decision trees and neural networks, which can more accurately learn the supervisor's policy compared to standard learners (i.e. linear models). We conjecture that their added expressiveness can help alleviate model error~\cite{vapnik1992principles}, which means the robot can now recover the expected supervisor's policy with enough data. In this case, the robot will converge to the supervisor's distribution \adnote{with HC sampling, and focusing demonstrations on the supervisor's distribution instead of the distribution induced by the current robot policy might be more advantageous.}. 

We \adnote{first }explore these ideas in a series of simulated robotic tasks. By varying the expressiveness of the robot's policy, we show on 100 randomly generated grid world environments that \adnote{the} performance gap between the two methods diminishes as the expressiveness is increased. We further demonstrate in a point mass control example how RC LfD can prevent convergence by visiting harder to learn areas. 

We then \adnote{compare the two in practice. We} performed a pilot study with 10 students at UC Berkeley, who were asked to use both RC and HC LfD to train a Zymark robot to singulate an object. We observed a statistically significant gap in the average performance of the policies trained by the two approaches. We found that RC was $40\%$ successful and HC was $60\%$ with the same amount of $60$ trials\adnote{i would talk here in terms of examples/data; trials in confusing} provided. 

\adnote{We conducted a post-analysis to identify possible reasons for this difference in performance. We found that participants are not able to match their own desired controls when asked to provide corrections to the robot, which helps support that indeed, RC is more difficult for human supervisors. We also found evidence that the policy learned with RC is more complex: we found a higher test loss for the RC policy, which can indicate harder to learn exaples were in the dataset.}
...
We finally present two theoretical contributions, which help analyze our empirical results. First, we show that \adnote{in some cases, }RC methods can prevent a robot from recovering the supervisor's policy when HC can. We also contribute a new analysis for the error accumulation in HC LfD, which demonstrates a tighter bound (in the time horizon of the task), for squared Euclidean losses and normally distributed stochastic policies,  than the rate shown \adnote{in prior work}.~\cite{ross2010efficient}.

Overall, our results suggest that for highly expressive deep learners, it may be more advantageous to get demonstrations using HC LfD than to ask the human supervisor for retroactive feedback along the robot's \adnote{current} policy.


\section{Related Work}
Below we summarize related work in HC and RC LfD and their theoretical insights. 

\noindent \textbf {\ns} \adnpte{(need much broader set of cites here, most people do HC LfD; also cite the lfd survey)} Pormeleau et al. used \ns to train a neural network to drive a car on the highway via demonstrations provided by a supervisor. To reduce the number of examples needed to perform a task, they synthetically generated images of the road and following labels~\cite{pomerleau1989alvinn}.  Schulman et al. used HC LfD for rope tying using kernelized interpolation as the policy class~\cite{schulman2016learning}. 
...
\noindent \textbf{\nc}
\nc has been used in numerous robotic examples, \adnote{including} flying a quadcopter through a forest where the state space is image data taken from an onboard sensor~\cite{ross2013learning}.

 Laskey et al. applied \nc to manipulation tasks such as surgical needle insertion \cite{laskeyshiv} and robotic de-cluttering, where a robot is given image data of a table with a variety of objects on it and must learn to push the obstacle objects aside to grasp a goal \cite{laskeyrobot}. Other successful examples have been teaching a robotic wheelchair to navigate to target positions and teaching a robot to follow verbal instructions to navigate across an office building \cite{kim2013maximum, duvallet2013imitation}. \adnote{(too much on us, just enumerate some examples with citations at the end of each example)}
...
All these approaches build theoretically upon the online optimization analysis that Ross et al. proposed~\cite{ross2010reduction}. In the online optimization algorithm, a bound for the error in the robot's policy is linear in $T$ for strongly convex losses (i.e.  regularized linear or kernelized regression). \adnote{The} analysis uses a regret style approach, which says\adnpte{informal; maybe implies?} the robot's error is bounded by the worst it could have done in hindsight. Thus, if a robot visits hard to learn states, \adnpte{its performance could be arbitrarily poor(? but if they bound performance wrt to hard states, that means that they also bound performance wrt easy states; need more here)}. We analyze this effect and show how it can prevent RC from converging to the supervisor's policy. 



\section{Problem Statement and Background}\label{sec:PS}
The goal of this work is to learn a policy that matches that of the supervisor on a specified task that demonstrations are collected on. \adnote{that's not the goal of this work, that is the goal of lfd; really this is a supervised lfd problem statement, not our problem statement, our problem statement is analyzing performance of HC vs RC;}
...
\noindent \textbf{\nc} \mlnote{need to shorten this}\adnote{I'll let you rework this section before I give comments} If $\theta* \in \Theta$, then as N grows, $
...
\adnote{We first provide an empirical comparison of HC and RC LfD. 
We start with} experiments in a  Grid World environment, which \adnote{enables} us to vary the robot's policy class over a large number of randomly generated environments\adnote{, assuming an ideal supervisor.}. Then we examine a linear dynamical system, which \adnote{we use to exemplify} a \adnote{potential }limitation \adnote{in }the ability of RC LfD to converge \adnote{(to what?) in cases where the policy over the entire state space is more complex than the policy over states visited under the supervisor distribution}. 

Finally, we \adnote{compare HC and RC on a real task. We }perform \adnote{a pilot study with} 10 participants, who try to teach a robot how to singulate, or separate an object from \adnote{clutter}. We used DAgger as the example of an RC algorithm in these experiments. 

\subsection{Varying Function Class}\label{sec:gdw}
In this experiment, we hypothesize that \adnpte{on random problems with a perfect simulated supervisor, }the performance gap of RC LfD diminishes as the robot's policy becomes more expressive. 

In a simulated Grid World, we have a robot that is trying to reach a goal state, at which it receives $+10$ reward. The robot receives $-10$ reward if it touches a penalty state. The robot also receives a $-0.02$ penalty for every blank state. The robot has a state space of $(x,y)$ coordinates and a set of actions consisting of $\lbrace$Left, Right, Forward, Backward, None$\rbrace$, where None indicates staying at the current \adnote{state}. The grid size for the environment \adnote{is} $30 \times 30$. $20\%$ of randomly drawn states are marked as a penalty, while only one is a goal state. For the transition dynamics, $p(\bx_{t+1}|\bx_{t},\bu_t)$, the robot goes to an adjacent state that does not correspond to the intended control with probability $0.2$.  The time horizon for the policy is $T=30$. 

We use Value Iteration to compute an optimal supervisor. The supervisor must learn to be robust to the noise in the dynamics, reach the goal state and then stay there. In all settings we provided the RC LfD with one \adnote{initial} demonstration from HC sampling \adnote{before iteratively rolling} out its policy. This initial demonstration set from HC sampling is common in methods like DAgger~\cite{rossreduction2010}.

 We run all trials over 100 randomly generated environments, which contain $270$ randomly selected penalty states and $1$ goal state. 

\adnote{We measure} normalized performance, where $1.0$ represents the expected performance of the optimal supervisor.\adnote{define performance!}

\noindent \textbf{Low Expressiveness} In Fig. \ref{fig:var}(a), we show the case when the robot's policy class does not contain the optimal supervisor's policy. \adnote{We} used a Linear SVM for the policy class representation, which is commonly used in \adnote{RC LfD} ~\cite{ross2010efficient,ross2010reduction,ross2013learning}. As \adnote{expected}, the RC sampling approach \adnote{outperforms} HC sampling, which is consistent with prior literature~\cite{ross2010efficient,ross2010reduction}.

This outcome suggests that when the robot is not able to learn the supervisor's policy, RC LfD has a \adnote{large} advantage. \adnote{Note that }neither method \adnote{is} able to converge to the supervisor's performance. 
...
As shown in Fig. \ref{fig:var}(b), RC LfD and HC LfD both converge to the supervisor, but at the same rate. \adnote{This suggests} that \adnote{the }advantage of using RC techniques diminishes once the robot's policy is more expressive. \adnote{When the robot is better able to learn the supervisor policy, it is no longer as useful to obtain demonstrations specifically on states visited by the robot's current policy as RC does.} We note that because the robot's policy class can represent the supervisor, both RC and HC methods converge to the supervisor's performance.


\noindent \textbf{Noisy supervisor}
\adnote{Real supervisors will not be perfect. }We \adnote{thus} the effects of noise on \adnote{performance of the two sampling techniques. This is particularly important for HC, because even an expressive learner might need many more demonstrations when the supervision is noisy, perhaps making samples on the current learned policy more useful}. Here we consider the case where noise is applied to observed label, thus the robot receive control labels that are $\bu = \pi_{\theta}(\bx) + \epsilon$,  where epsilon is an i.i.d distribution that selects a random control with probability $0.3$.

We use the  decision tree  of depth $100$, which due to its large function class is more susceptible to the noisy supervisor. We then compare the performance of HC vs. RC LfD. As shown, both method are able to converge to the expected supervisor's normalized performance because they can average out the noise in the labels with enough data. Though it  takes more data \adnote{than before, }both methods (HC and RC) converge at a similar rate to the true expected supervisor. \adnote{Of course, real supervisors are not necessarily i.i.d noisy, and our real-problem experiment will compare the two methods in practice.}




\subsection{Algorithmic Convergence }
In this experiment, we \adnote{show an example in which} HC methods converge to the superivsor's performance and RC does not. \adnote{We construct an environment that is representative of our insight that, in real problems, the policy on \emph{all} states (including recovery behaviors) is more complex than the policy on the supervisor distribution.} 

We consider the example where a robot needs to learn to get to a location in a 2D continuous world domain. The robot is represented as a point mass with linear dynamics and the supervisor is computed using the infinite time horizon LQG, which results in a linear policy. 
