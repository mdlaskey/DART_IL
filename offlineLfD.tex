...
\begin{align*}
	E_{p(\tau \mid \theta_{d}^N) } J(\theta_{d}^N) > E_{p(\tau \mid \theta_{p}^N)) } J(\theta_{p}^N) = E_{p(\tau \mid \theta^*)} J(\theta^*)
\end{align*} 
\noindent for all $N > N_0$ and $M > M_0$ and some $M_0, N_0 \in \mathbb{Z}$.
In other words, DAgger may fail to converge in cases where passive LfD matches the expert exactly.
\end{theorem}
