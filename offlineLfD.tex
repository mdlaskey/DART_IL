...
In the supervised learning approach to LfD, a robot's policy, or a mapping from the state space to controls, is learned via regression or classification. The traditional approach to LfD is to collect supervisor demonstrations on the states the supervisor is likely to visit~\cite{pomerleau1989alvinn}. We define this as Human-Centric (HC) because the human controls what states the robot will observe. However, due to learning error, the robot's policy will not match the supervisor.  The robot will then visit different states when performing the task. A common trend in LfD is adaptivity, which is to have the supervisor retroactively provide feedback (or the correct control signal) to the robot in the states visited, this refers to DAgger, SHIV and variants~\cite{ross2010efficient,ross2010reduction,laskeyrobot,laskeyshiv,he2012imitation}. We refer to this approach as Robot-Centric (RC) LfD because the robot influences the choice of the label. 


While RC has significant advantages, it comes at a cost. One cost is the challenge posed to a human supervisor.  RC methods have a human provide retroactive feedback to the robot without observing the effect of their suggested control.  One way to view this challenge is that a human supervisor must predict how the world will change given their control. In comparison for HC, the human only performs teleoperation, a form of feedback control. \mlnote{motivate this with HRI related work} 

 Second, having the robot visit states due to previous errors can cause the human to provide examples that are harder to learn.  These situations are common when the robot veers into a state where the recovery behavior adds complexity to the task. Adding these difficult examples to the dataset could reduce the robot's ability to match the supervisor. 

We explore the value of the extra overhead of RC for emerging classes of highly-expressive deep learning models such as decision trees and neural networks, which can more accurately learn the supervisor's policy compared to standard learners (i.e. linear models). We conjecture that their added expressiveness can help alleviate model error~\cite{vapnik1992principles}, which means the robot can now recover the expected supervisor's policy with enough data. In this case, the robot will converge to the supervisor's distribution. 
