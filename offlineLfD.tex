...
All these approaches build theoretically upon the online optimization analysis that Ross et al. proposed~\cite{ross2010reduction}. In the online optimization algorithm, a bound for the error in the robot's policy is linear in $T$ for strongly convex losses (i.e.  regularized linear or kernelized regression). \adnote{The} analysis uses a regret style approach, which says\adnote{informal maybe implies?} the robot's error is bounded by the worst it could have done in hindsight. Thus, if a robot visits hard to learn states, \adnote{its performance could be arbitrarily poor(? but if they bound performance wrt to hard states, that means that they also bound performance wrt easy states; need more here)}. We analyze this effect and show how it can prevent RC from converging to the supervisor's policy. 
...
In this experiment, we hypothesize that \adnote{on random problems with a perfect simulated supervisor, }the performance gap of RC LfD diminishes as the robot's policy becomes more expressive. 
