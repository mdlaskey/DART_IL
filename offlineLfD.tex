...
Below we summarize related work in passive and adaptive LfD and their theoretical insights. 
...
We assume a probability density over initial states $p(\bx_0)$. The environment of a task is thus defined as the tuple, $\lbrace \mathcal{X}, \mathcal{U}, p(\bx_0),p(\bx_{t+1}|\bu_{t}, \bx_t)\rbrace$.
...
In the passive LfD setting a robot is trained on the states visited by the expert. However, at run time it can encounter a different distribution of states due to not perfectly matching the expert's policy. Ross et al. showed that given a time horizon, $T$, the worst case error scales quadritically (i.e. $O(T^2E_{p(\bx|\theta^*)} l(\hat{\theta})$) when executing the robot's policy~\cite{ross2010efficient}. Note according to the notation of Ross et al., $TE_{p(\bx|\theta^*)} l(\hat{\theta}) = E_{p(\tau|\theta)} J(\theta)$. We present a new analysis for a specific class of policies defined below that shows a rate of $O(T\sqrt{TE_{p(\bx|\theta^*)} l(\hat{\theta})})$, which suggests a tamer rate in the time time horizon. 
...
&= T\sqrt{T \frac{1}{4\sigma} E_{p(\bx|\theta^*)} l(\hat{\theta})}
\end{align}
...
It is important to note that our bound can be larger than the original analysis provided by Ross et al. when  $E_{p(\bx|\theta^*)} l(\hat{\theta})$ is very low. However, when not enough demonstrations have been collected or model error exists $E_{p(\bx|\theta^*)} l(\hat{\theta})$ could be quite large. Thus, suggesting that passive Lfd has more robustness than originally thought. 
