...
\begin{align*}
	E_{p(\tau \mid \theta_{d}^K) } J(\theta_{d}^K) > E_{p(\tau \mid \theta_{p}^N) } J(\theta_{p}^N) = E_{p(\tau \mid \theta^*)} J(\theta^*)
\end{align*} 
\noindent for all $N > N_0$ and $K > K_0$ and some $K_0, N_0 \in \mathbb{Z}$.
In other words, RC may fail to converge in cases where HC LfD matches the supervisor exactly.
\end{theorem}
...
\begin{figure}
\centering
\includegraphics{f_figs/cor_dif.eps}
\caption{
    \footnotesize
